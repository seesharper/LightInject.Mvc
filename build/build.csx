...
private const string projectName = "LightInject.Mvc";

private const string portableClassLibraryProjectTypeGuid = "{786C830F-07A1-408B-BD7F-6EE04809D6DB}";
private const string csharpProjectTypeGuid = "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}";

string pathToSourceFile = @"..\src\LightInject.Mvc\LightInject.Mvc.cs";
...
WriteLine("LightInject.Mvc version {0}" , version);
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package/LightInject.Mvc.Source.nuspec");	
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Binary/package/LightInject.Mvc.nuspec");
	PatchNugetVersionInfo(pathToMetadataFile, version);
	NuGet.CreatePackage(pathToMetadataFile, pathToBuildDirectory);
}
...
{
	string pathToMetadata = "../src/LightInject.Mvc/NuGet";
	string pathToPackageDirectory = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package");	
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Mvc.Source.nuspec");	
	string pathToSourceFile = "tmp/" + frameworkMoniker + "/Source/LightInject.Mvc";
	string pathToDestination = Path.Combine(pathToPackageDirectory, "content/" + packageDirectoryName + "/LightInject.Mvc");
	RoboCopy(pathToSourceFile, pathToDestination, "LightInject.Mvc.cs");
	FileUtils.Rename(Path.Combine(pathToDestination, "LightInject.Mvc.cs"), "LightInject.Mvc.cs.pp");
	ReplaceInFile(@"namespace \S*", "namespace $rootnamespace$.LightInject.Mvc", Path.Combine(pathToDestination, "LightInject.Mvc.cs.pp"));
}
...
private void CopyBinaryFile(string frameworkMoniker, string packageDirectoryName)
{
	...
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Mvc.nuspec");
	...
	RoboCopy(pathToBinaryFile, pathToDestination, "LightInject.Mvc.*");
}

private string ResolvePathToBinaryFile(string frameworkMoniker)
{
	var pathToBinaryFile = Directory.GetFiles("tmp/" + frameworkMoniker + "/Binary/LightInject.Mvc/bin/Release","LightInject.Mvc.dll", SearchOption.AllDirectories).First();
...
{
	string pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Mvc");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Mvc.Tests");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Mvc");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Mvc.Tests");
	NuGet.Restore(pathToProjectDirectory);
...
{
	string pathToTestAssembly = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Mvc.Tests/bin/Release/LightInject.Mvc.Tests.dll");
...
{	
    ...
    MsTest.RunWithCodeCoverage(pathToTestAssembly, pathToPackagesFolder,pathToTestAdapterDirectory, "LightInject.Mvc.dll");
}
...
private void InitializeNugetBuildDirectory(string frameworkMoniker)
{
	...
	{
		var pathToJsonTemplateFile = Path.Combine(pathToBinary, "LightInject.Mvc/project.json_");
		var pathToJsonFile = Path.Combine(pathToBinary, "LightInject.Mvc/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
		pathToJsonTemplateFile = Path.Combine(pathToSource, "LightInject.Mvc/project.json_");
		pathToJsonFile = Path.Combine(pathToSource, "LightInject.Mvc/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
	}				  
}
...
private void Internalize(string frameworkMoniker)
{
	...
	string pathToSourceFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + "/Source/LightInject.Mvc/LightInject.Mvc.cs");
...
{
	string pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Mvc/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
	
	pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Mvc/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
}
...
{	
	var pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Binary\LightInject.Mvc\Properties\AssemblyInfo.cs");	
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);
	pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Source\LightInject.Mvc\Properties\AssemblyInfo.cs");
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);	
...
private void PatchInternalsVisibleToAttribute(string pathToAssemblyInfo)
{
	...
	sb.AppendLine(@"[assembly: InternalsVisibleTo(""LightInject.Mvc.Tests"")]");
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Mvc/LightInject.Mvc.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Mvc/LightInject.Mvc.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
...
private void SetTargetFrameworkProfile()
{
	...
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, @"PCL_111/Binary/LightInject.Mvc/LightInject.Mvc.csproj");
	XDocument xmlDocument = XDocument.Load(pathToProjectFile);	
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Binary\LightInject.Mvc.Tests\LightInject.Mvc.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);	
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Source\LightInject.Mvc.Tests\LightInject.Mvc.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
}